...
class func:
    def setup_window(self, mode):
        ...
        if mode == 'advanced':
            self.winwid += self.btnwid * 2  # 多機能モードでは幅を拡張
    ...
    def setup_styles(self):
        """言語に応じたスタイルを設定"""
        font_sizes = {
            'JA-Jp': {'stdButton': 20, 'stdButton2': 18, 'stdButton3': 14, 'stdButton4': 10, 'stdLabel': 15},
            'US-En': {'stdButton': 20, 'stdButton2': 18, 'stdButton3': 14, 'stdButton4': 12, 'stdLabel': 15}
        }
...
class main_win(func):
    def __init__(self):
        ...
        self.console = ttk.Label(self.main_win, relief="sunken", style="stdLabel.TLabel", anchor=tk.N, textvariable=self.textoutput)
        self.console.place(x=0, y=0, width=360, height=180)
